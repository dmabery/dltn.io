...
const PostList = ({post, index, key}) => {
    return (
            <div className="rounded-lg dark:border-gray-700 mb-4 relative" key={key}>
            <div>
                <Link href={'/essays/' + post.slug} passHref key={key}>
                    <h5 className="text-2xl font-bold tracking-tight text-gray-900">{post.frontMatter.title}</h5>
                </Link>
                <p className="mb-2 font-normal text-gray-700 dark:text-gray-400">{post.frontMatter.description}</p>
                <p className="mb-4 text-sm text-gray-400">Published on: {post.frontMatter.date}</p>
                <Link href={'/essays/' + post.slug} passHref key={key} class="inline-flex items-center text-sm font-medium text-center text-slate-500 hover:text-slate-900">
