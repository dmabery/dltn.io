...
const Subscribe = ({ title, placeholder }) => {
  ...
  const formClass = classNames({
    "flex flex-col gap-1 font-sansSerif items-center rounded-md": true,
    "border-gray-100": isLoading,
  });

  const inputClass = classNames({
    "text-sm appearance-none border border-slate-500 w-full text-gray-900/80 py-2 px-2 leading-tight focus:border-gray-900": true,
    "opacity-100 cursor-not-allowed": isLoading,
  });

  const btnClass = classNames({
    "flex-shrink-0 md:text-black md:bg-neutral-100 w-full bg-neutral-900 hover:bg-neutral-900/80 text-sm text-white py-2 px-4 transition-all": true,
