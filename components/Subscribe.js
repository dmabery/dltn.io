...
const Subscribe = ({ title, placeholder }) => {
  ...
  const inputClass = classNames({
    "text-sm appearance-none border border-slate-500 w-3/4 text-gray-900/80 py-2 px-2 leading-tight focus:border-gray-900": true,
    "opacity-100 cursor-not-allowed": isLoading,
  });

  const btnClass = classNames({
    "flex-shrink-0 w-1/4 bg-neutral-900 hover:bg-neutral-900/80 text-sm text-white py-2 px-4 transition-all": true,
